...
func getLast24hCandlestickAskBid() {
	...
	if err != nil {
		...
		log.Info("Bid=%+v, Ask=%+v", resp.Bid, resp.Ask)
	}
}
...
func getLast24hCandlesticks() {
	...
	if err != nil {
		...
		for _, tick := range resp {
			log.Info("Symbol: %s, High: %v, Low: %v, Ask[%v, %v], Bid[%v, %v]",
...
func getLatestTrade() {
	...
	if err != nil {
		...
		for _, trade := range resp.Data {
			log.Info("Id=%v, Price=%v", trade.Id, trade.Price)
		}
	}
}
...
func getLast24hCandlestick() {
	...
	if err != nil {
		...
		log.Info("Close=%v, Open=%v", resp.Close, resp.Open)
	}
}
